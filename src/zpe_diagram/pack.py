...
from .constants import DEFAULT_VERSION, MODE_EXTENSION
...
def _ext_word(payload: int) -> int:
    return (MODE_EXTENSION << 18) | (DEFAULT_VERSION << 16) | (payload & 0xFFFF)
...
def is_visual_word(word: int) -> bool:
    if not isinstance(word, int):
        return False
    return ((word >> 18) & 0x3) == MODE_EXTENSION and bool((word & 0xFFFF) & VISUAL_TYPE_BIT)
...
def pack_diagram_paths(
    ...
    *,
    ...
    Subtype fields live in the low bits. Optional style suffix encodes exactly
    three words on the closing boundary.
    """
...
def unpack_diagram_words(words: Iterable[int]) -> List[StrokePath]:
    ...
    while i < len(in_words):
        ...
        if mode != MODE_EXTENSION or version != DEFAULT_VERSION or not (payload & VISUAL_TYPE_BIT):
            i += 1
            continue
