...
def _require_svgpathtools():
    try:
        ...
        raise ImportError(
            ...
            "Install via `pip install zpe-diagram`."
        ) from exc
    return svg2paths2
...
def _parse_style(attr: dict) -> Tuple[str | None, float | None, str | None, str | None]:
    stroke = attr.get("stroke")
    stroke_width = attr.get("stroke-width")
    fill = attr.get("fill")
    dash = attr.get("stroke-dasharray")
    ...
    if dash is not None:
        dash = dash.strip()
        if not dash or dash.lower() == "none":
            dash = None
    return stroke, sw, fill, dash
...
def _collect_inherited_styles(svg_text: str) -> List[dict]:
    ...
    def walk(elem, inherited: dict):
        ...
        for key in ("stroke", "fill", "stroke-width", "stroke-dasharray", "transform"):
...
def svg_to_polylines(svg_text: str, max_seg: float = 2.0, canvas_size: int = 256, padding: int = 4) -> List[PolylineShape]:
    ...
    for idx, (path, attr) in enumerate(zip(paths, attrs)):
        ...
        stroke, stroke_width, fill, dash = _parse_style(attr)
        if stroke is None:
        ...
        if dash is None:
            inherited_dash = inherited.get("stroke-dasharray")
            if inherited_dash is not None:
                dash = str(inherited_dash).strip() or None
        if dash is not None and dash.lower() == "none":
            dash = None
